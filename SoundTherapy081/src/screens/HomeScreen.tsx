...
export const HomeScreen: React.FC = () => {
  ...
  useEffect(() => {
    // 【关键修复】AudioService 已在 App.tsx 中统一初始化，这里不再重复调用 setupPlayer
    const init = async () => {
      try {
        console.log('[AudioService-DIAGNOSE] [HomeScreen] 开始检查音频服务状态');
        
        const audioService = AudioService.getInstance();
        
        // 【状态守卫】检查 AudioService 是否已准备好
        if (!audioService.isReady()) {
          console.warn('[AudioService-DIAGNOSE] [HomeScreen] ⚠️ AudioService 未准备好，跳过预加载');
          return;
        }
        
        console.log('[AudioService-DIAGNOSE] [HomeScreen] ✅ AudioService 已准备好');
        
        // 如果当前没有在播放，则预加载第一个场景（不自动播放）
        if (!audioService.isPlaying()) {
          console.log('[AudioService-DIAGNOSE] [HomeScreen] 预加载第一个场景：', SCENES[0].id);
          await audioService.loadAudio(SCENES[0], false);
          console.log('[AudioService-DIAGNOSE] [HomeScreen] ✅ 场景加载完成');
        }
        console.log('[AudioService-DIAGNOSE] [HomeScreen] ====== 音频服务检查完成 ======');
      } catch (e) {
        console.error('[AudioService-DIAGNOSE] [HomeScreen] ❌ Audio check failed:', e);
