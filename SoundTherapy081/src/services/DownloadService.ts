...
const DOWNLOAD_CONNECTION_TIMEOUT = 15000;
const DOWNLOAD_READ_TIMEOUT = 30000;
const DOWNLOAD_STALL_TIMEOUT = 15000;
  ...
  async checkAndDownload(onProgress: (p: DownloadProgress) => void) {
    ...
    const downloadSingleFile = async (asset: any): Promise<boolean> => {
      ...
      for (let attempt = 0; attempt < MAX_RETRIES_PER_FILE; attempt++) {
        if (attempt > 0) {
          console.log(`[App-Download] ⚠️ 重试 ${asset.filename} (第${attempt + 1}次)`);
